...
class appNotice:
    ...
    def aplNotice(self, programName, noticeMessage1, noticeMessage2):
        try:
            # settings_secret.yml内容が空か確認 → settings_secret.yml内容を取得
            global settingsYaml
            if not settingsYaml:
    ...
    ##################################################################################################################################################
    # settings_secret.yml内容セットメソッド
    # @param  self               : self
    ##################################################################################################################################################
    def setSettingsYaml(self):

        # settings_secret.yml内容セット
        global settingsYaml
        with open('../settings/settings_secret.yml', 'r') as yml:
            settingsYaml = yaml.safe_load(yml)

    ##################################################################################################################################################
    # settings_secret.yml内容ゲットメソッド
    # @param  self               : self
    # @return settings_secret.yml : settings_secret.yml内容
    ##################################################################################################################################################
    def getSettingsYaml(self):

        # settings_secret.yml内容ゲット
        return settingsYaml
