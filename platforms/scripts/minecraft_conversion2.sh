...
#-----------------------------------------------------------------------------------------------------------------------------------------------------
# 【修正履歴】
# V-001      : 2025/08/22                 Gamer-Iris   新規作成
# 
######################################################################################################################################################
...
DATABASES_DATABASE1_DATABASENAME=`cat ~/Linux/platforms/settings/settings.yml | yq eval '.databases.database1.databasename'`
DATABASES_DATABASE1_USERNAME=`cat ~/Linux/platforms/settings/settings.yml | yq eval '.databases.database1.username'`
DATABASES_DATABASE1_PASSWORD=`cat ~/Linux/platforms/settings/settings.yml | yq eval '.databases.database1.password'`
DATABASES_DATABASE2_DATABASENAME=`cat ~/Linux/platforms/settings/settings.yml | yq eval '.databases.database2.databasename'`
DATABASES_DATABASE2_USERNAME=`cat ~/Linux/platforms/settings/settings.yml | yq eval '.databases.database2.username'`
DATABASES_DATABASE2_PASSWORD=`cat ~/Linux/platforms/settings/settings.yml | yq eval '.databases.database2.password'`
...
CONVERSION_FULL_PATH11_1="${CONVERSION_FOLDER1}"/plugins/floodgate/"${CONVERSION_FILE_NAME11}"
CONVERSION_FULL_PATH11_2="${CONVERSION_FOLDER1}"/plugins/Geyser-BungeeCord/"${CONVERSION_FILE_NAME11}"
BACKUP_FULL_PATH11_1="${CONVERSION_FOLDER1}"/plugins/floodgate/"${CONVERSION_FILE_NAME11}_bk"
BACKUP_FULL_PATH11_2="${CONVERSION_FOLDER1}"/plugins/Geyser-BungeeCord/"${CONVERSION_FILE_NAME11}_bk"
...
CONVERSION_FULL_PATH12_1="${CONVERSION_FOLDER1}"/plugins/LuckPerms/"${CONVERSION_FILE_NAME12}"
CONVERSION_FULL_PATH12_2="${CONVERSION_FOLDER2}"/plugins/LuckPerms/"${CONVERSION_FILE_NAME12}"
CONVERSION_FULL_PATH12_3="${CONVERSION_FOLDER3}"/plugins/LuckPerms/"${CONVERSION_FILE_NAME12}"
BACKUP_FULL_PATH12_1="${CONVERSION_FOLDER1}"/plugins/LuckPerms/"${CONVERSION_FILE_NAME12}_bk"
BACKUP_FULL_PATH12_2="${CONVERSION_FOLDER2}"/plugins/LuckPerms/"${CONVERSION_FILE_NAME12}_bk"
BACKUP_FULL_PATH12_3="${CONVERSION_FOLDER3}"/plugins/LuckPerms/"${CONVERSION_FILE_NAME12}_bk"
WORK_FULL_PATH12_1="${WORK_DIR}"/"${CONVERSION_FILE_NAME12}_12_1"
WORK_FULL_PATH12_2="${WORK_DIR}"/"${CONVERSION_FILE_NAME12}_12_2"
WORK_FULL_PATH12_3="${WORK_DIR}"/"${CONVERSION_FILE_NAME12}_12_3"
## LunaChatConfig
CONVERSION_FILE_NAME13=config.yml
CONVERSION_FULL_PATH13_1="${CONVERSION_FOLDER2}"/plugins/LunaChat/"${CONVERSION_FILE_NAME13}"
CONVERSION_FULL_PATH13_2="${CONVERSION_FOLDER3}"/plugins/LunaChat/"${CONVERSION_FILE_NAME13}"
BACKUP_FULL_PATH13_1="${CONVERSION_FOLDER2}"/plugins/LunaChat/"${CONVERSION_FILE_NAME13}_bk"
BACKUP_FULL_PATH13_2="${CONVERSION_FOLDER3}"/plugins/LunaChat/"${CONVERSION_FILE_NAME13}_bk"
WORK_FULL_PATH13_1="${WORK_DIR}"/"${CONVERSION_FILE_NAME13}_13_1"
WORK_FULL_PATH13_2="${WORK_DIR}"/"${CONVERSION_FILE_NAME13}_13_2"
...
#*****************************************************************************************************************************************************
    "STEP080")
      # EXEC------------------------------------------------------------------------------------------------------------------------------------------
      yq eval -i '.username-prefix = "BE_"' "${WORK_FULL_PATH11_1}" && \
      yq eval -i '.remote.auth-type = "floodgate"' "${WORK_FULL_PATH11_2}"
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [[ ${RTN_CD} -eq 0 ]]; then
        NSTEP="STEP090"
...
#*****************************************************************************************************************************************************
    "STEP100")
      ...
      cp -p "${CONVERSION_FULL_PATH12_3}" "${BACKUP_FULL_PATH12_3}" && cp -p "${CONVERSION_FULL_PATH12_3}" "${WORK_FULL_PATH12_3}"
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [[ ${RTN_CD} -eq 0 ]]; then
        NSTEP="STEP110"
...
#*****************************************************************************************************************************************************
    "STEP110")
      # EXEC------------------------------------------------------------------------------------------------------------------------------------------
      yq eval -i '.use-server-uuid-cache = true' "${WORK_FULL_PATH12_1}" && \
      yq eval -i ".storage-method = \"${DATABASES_OSS}\"" "${WORK_FULL_PATH12_1}" && \
      yq eval -i ".data.address = \"${DATABASES_ADDRESS}\"" "${WORK_FULL_PATH12_1}" && \
      yq eval -i ".data.database = \"${DATABASES_DATABASE3_DATABASENAME}\"" "${WORK_FULL_PATH12_1}" && \
      yq eval -i ".data.username = \"${DATABASES_DATABASE3_USERNAME}\"" "${WORK_FULL_PATH12_1}" && \
      yq eval -i ".data.password = \"${DATABASES_DATABASE3_PASSWORD}\"" "${WORK_FULL_PATH12_1}" && \
      yq eval -i '.server = "server1"' "${WORK_FULL_PATH12_2}" && \
      yq eval -i '.use-server-uuid-cache = true' "${WORK_FULL_PATH12_2}" && \
      yq eval -i ".storage-method = \"${DATABASES_OSS}\"" "${WORK_FULL_PATH12_2}" && \
      yq eval -i ".data.address = \"${DATABASES_ADDRESS}\"" "${WORK_FULL_PATH12_2}" && \
      yq eval -i ".data.database = \"${DATABASES_DATABASE3_DATABASENAME}\"" "${WORK_FULL_PATH12_2}" && \
      yq eval -i ".data.username = \"${DATABASES_DATABASE3_USERNAME}\"" "${WORK_FULL_PATH12_2}" && \
      yq eval -i ".data.password = \"${DATABASES_DATABASE3_PASSWORD}\"" "${WORK_FULL_PATH12_2}" && \
      yq eval -i '.server = "server2"' "${WORK_FULL_PATH12_3}" && \
      yq eval -i '.use-server-uuid-cache = true' "${WORK_FULL_PATH12_3}" && \
      yq eval -i ".storage-method = \"${DATABASES_OSS}\"" "${WORK_FULL_PATH12_3}" && \
      yq eval -i ".data.address = \"${DATABASES_ADDRESS}\"" "${WORK_FULL_PATH12_3}" && \
      yq eval -i ".data.database = \"${DATABASES_DATABASE3_DATABASENAME}\"" "${WORK_FULL_PATH12_3}" && \
      yq eval -i ".data.username = \"${DATABASES_DATABASE3_USERNAME}\"" "${WORK_FULL_PATH12_3}" && \
      yq eval -i ".data.password = \"${DATABASES_DATABASE3_PASSWORD}\"" "${WORK_FULL_PATH12_3}"
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [[ ${RTN_CD} -eq 0 ]]; then
        NSTEP="STEP120"
...
#*****************************************************************************************************************************************************
    "STEP120")
      ...
      cp -p "${WORK_FULL_PATH12_3}" "${CONVERSION_FULL_PATH12_3}"
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [[ ${RTN_CD} -eq 0 ]]; then
        NSTEP="STEP130"
...
#*****************************************************************************************************************************************************
    "STEP130")
      # EXEC------------------------------------------------------------------------------------------------------------------------------------------
      cp -p "${CONVERSION_FULL_PATH13_1}" "${BACKUP_FULL_PATH13_1}" && cp -p "${CONVERSION_FULL_PATH13_1}" "${WORK_FULL_PATH13_1}" && \
      cp -p "${CONVERSION_FULL_PATH13_2}" "${BACKUP_FULL_PATH13_2}" && cp -p "${CONVERSION_FULL_PATH13_2}" "${WORK_FULL_PATH13_2}"
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [[ ${RTN_CD} -eq 0 ]]; then
        NSTEP="STEP140"
...
#*****************************************************************************************************************************************************
    "STEP140")
      # EXEC------------------------------------------------------------------------------------------------------------------------------------------
      yq eval -i '.japanizeType = "GoogleIME"' "${WORK_FULL_PATH13_1}" && \
      yq eval -i '.japanizeType = "GoogleIME"' "${WORK_FULL_PATH13_2}"
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [[ ${RTN_CD} -eq 0 ]]; then
        NSTEP="STEP150"
...
#*****************************************************************************************************************************************************
    "STEP150")
      # EXEC------------------------------------------------------------------------------------------------------------------------------------------
      cp -p "${WORK_FULL_PATH13_1}" "${CONVERSION_FULL_PATH13_1}" && \
      cp -p "${WORK_FULL_PATH13_2}" "${CONVERSION_FULL_PATH13_2}"
      # RETURN----------------------------------------------------------------------------------------------------------------------------------------
      RTN_CD=$?
      if [[ ${RTN_CD} -eq 0 ]]; then
        NSTEP="JOBEND"
...
rm -r "${WORK_FULL_PATH12_3}"
rm -r "${WORK_FULL_PATH13_1}"
rm -r "${WORK_FULL_PATH13_2}"
