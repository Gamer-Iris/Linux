...
print_case_debug() {
  ...
  if [[ -n "${curl_lookup}" ]]; then
    echo "command -v curl: ${curl_lookup}" >&2
  else
    echo "command -v curl: (not found)" >&2
  fi
  if [[ -n "${yq_lookup}" ]]; then
    echo "command -v yq: ${yq_lookup}" >&2
  else
    echo "command -v yq: (not found)" >&2
  fi
...
assert_publish_case_message() {
  ...
  case "${case_name}" in
    no-publish)
      ...
      ;;
    auto-disabled)
      assert_contains "${haystack}" "github.dispatch_build_release=false" "${case_name}"
      assert_contains "${haystack}" "dispatch" "${case_name}"
      ;;
    token-missing)
      assert_contains "${haystack}" "github.owner / github.repo / github.token" "${case_name}"
      assert_contains "${haystack}" "token 文字列はログやチャットへ貼らないでください" "${case_name}"
      ;;
    workflow-get-fail)
      assert_contains "${haystack}" "workflow の参照に失敗" "${case_name}"
      assert_contains "${haystack}" "Actions: Read" "${case_name}"
      ;;
    dispatch-fail)
      assert_contains "${haystack}" "workflow" "${case_name}"
      assert_contains "${haystack}" "dispatch" "${case_name}"
      assert_contains "${haystack}" "Actions: Read and write" "${case_name}"
      ;;
    dry-run)
      assert_contains "${haystack}" "dry-run" "${case_name}"
      assert_contains "${haystack}" "dispatch" "${case_name}"
      ;;
    auto-enabled-success|publish-force-success)
...
query="$2"
case "${query}" in
  ".github.dispatch_build_release // false") printf '%s\n' "${STUB_GITHUB_DISPATCH_BUILD_RELEASE:-false}" ;;
  ".github.enabled // false") printf '%s\n' "${STUB_GITHUB_ENABLED:-true}" ;;
  ".github.owner // \"\"") printf '%s\n' "${STUB_GITHUB_OWNER:-Gamer-Iris}" ;;
  ".github.repo // \"\"") printf '%s\n' "${STUB_GITHUB_REPO:-Linux}" ;;
  ".github.token // \"\"") printf '%s\n' "${STUB_GITHUB_TOKEN-stub-token}" ;;
  ".github.build_release_workflow // \"build-release.yml\"") printf '%s\n' "${STUB_GITHUB_BUILD_RELEASE_WORKFLOW:-build-release.yml}" ;;
  ".github.build_release_ref // \"main\"") printf '%s\n' "${STUB_GITHUB_BUILD_RELEASE_REF:-main}" ;;
  *)
    echo "unexpected yq query: ${query}" >&2
    exit 1
    ;;
esac
EOF
...
printf 'method=%s url=%s body=%s\n' "\${method}" "\${url}" "\${body}" >> "\${CALLS_FILE}"

case "\${MODE}:\${method}" in
  workflow-get-fail:GET)
    printf '403'
    exit 0
    ;;
  dispatch-fail:POST)
...
run_publish_case() {
  ...
  local dispatch_build_release="$5"
  local token_value="$6"
  local expected_rc="$7"
  local check_output="$8"
  local bin_dir="${TMP_DIR}/${case_name}/bin"
  local calls_file="${TMP_DIR}/${case_name}/curl.calls"
  local log_file="${TMP_DIR}/${case_name}/setup.log"
  local settings_file="${TMP_DIR}/${case_name}/settings_secret.yml"
  local case_path="${bin_dir}:${ORIGINAL_PATH}"
  local curl_lookup=""
  local yq_lookup=""
  local output=""
  local rc=0
  ...
  write_yq_stub "${bin_dir}"
  : > "${calls_file}"
  : > "${settings_file}"

  write_curl_stub "${bin_dir}" "${curl_mode}" "${calls_file}"

  curl_lookup="$(PATH="${case_path}" command -v curl 2>/dev/null || true)"
  yq_lookup="$(PATH="${case_path}" command -v yq 2>/dev/null || true)"

  set +e
  output="$(
    {
      PATH="${case_path}"
      export STUB_GITHUB_ENABLED="true"
      export STUB_GITHUB_OWNER="Gamer-Iris"
      export STUB_GITHUB_REPO="Linux"
      export STUB_GITHUB_TOKEN="${token_value}"
      export STUB_GITHUB_DISPATCH_BUILD_RELEASE="${dispatch_build_release}"
      export STUB_GITHUB_BUILD_RELEASE_WORKFLOW="build-release.yml"
      export STUB_GITHUB_BUILD_RELEASE_REF="main"
  ...
  case "${case_name}" in
    dry-run|no-publish|auto-disabled|token-missing)
      if [[ -s "${calls_file}" ]]; then
        fail "${case_name}: curl を呼び出してはいけないケースで呼び出されました"
      fi
      ;;
    auto-enabled-success|publish-force-success|dispatch-fail)
      local calls
      calls="$(<"${calls_file}")"
      assert_contains "${calls}" "/repos/Gamer-Iris/Linux/actions/workflows/build-release.yml" "${case_name}"
      assert_contains "${calls}" "method=GET" "${case_name}"
      assert_contains "${calls}" "/repos/Gamer-Iris/Linux/actions/workflows/build-release.yml/dispatches" "${case_name}"
      assert_contains "${calls}" "method=POST" "${case_name}"
      assert_contains "${calls}" '"deploy_to_servers":"false"' "${case_name}"
    ...
    workflow-get-fail)
      ...
      calls="$(<"${calls_file}")"
      assert_contains "${calls}" "method=GET" "${case_name}"
      assert_not_contains "${calls}" "method=POST" "${case_name}"
      ;;
  esac

  if [[ -n "${token_value}" ]]; then
    assert_not_contains "${output}" "${token_value}" "${case_name}"
  fi

  if [[ "${FAIL_COUNT}" -gt "${failures_before}" ]]; then
    print_case_debug "${case_name}" "${output}" "${case_path}" "${curl_lookup}" "${yq_lookup}"
  fi
}
...
maintenance = (repo_root / ".github/workflows/minecraft-maintenance.yml").read_text(encoding="utf-8")
build_release = (repo_root / ".github/workflows/build-release.yml").read_text(encoding="utf-8")
setup_sh = (repo_root / "platforms/setup/setup.sh").read_text(encoding="utf-8")
readme = (repo_root / "README.md").read_text(encoding="utf-8")
readme_plain = re.sub(r"[`'\"“”‘’]", "", readme)
readme_plain = re.sub(r"\s+", " ", readme_plain)

if "PUBLISH_RELEASE=auto" not in setup_sh:
    fail("setup.sh は PUBLISH_RELEASE=auto を既定値にしてください")
...
for forbidden in (
    ...
    if forbidden in setup_sh:
        fail(f"setup.sh の Release dispatch path に GitHub command line dependency を残さないでください: {forbidden}")

for required in (
    ".github.owner",
    ".github.repo",
    ".github.token",
    ".github.dispatch_build_release",
    "/actions/workflows/",
    "/dispatches",
    "deploy_to_servers",
    "output = /dev/null",
    "token 文字列はログやチャットへ貼らないでください",
):
    if required not in setup_sh:
        fail(f"setup.sh の GitHub API dispatch 実装に必要な要素がありません: {required}")

if 'deploy_to_servers":"false' not in setup_sh:
    fail("setup.sh は deploy_to_servers=false で workflow dispatch してください")

if 'publish_release="${PUBLISH_RELEASE:-auto}"' not in (repo_root / "platforms/setup/lib/validation.sh").read_text(encoding="utf-8"):
    fail("validation.sh は PUBLISH_RELEASE を set -u 安全に参照してください")

validation_sh = (repo_root / "platforms/setup/lib/validation.sh").read_text(encoding="utf-8")
if '${publish_release}" != "0"' not in validation_sh or '${publish_release}" == "1"' not in validation_sh:
    fail("validation.sh は --publish / --no-publish と dispatch_build_release の precheck 条件を runtime と整合させてください")
...
require_readme_patterns(
    "README.md は setup.sh all の Release dispatch が settings_secret.yml の github 設定に従うことを説明してください",
    (
        r"setup\.sh\s+all",
        r"github\.dispatch_build_release",
        r"github\.owner",
        r"github\.repo",
        r"github\.token",
        r"Actions:\s*Read\s+and\s+write|Actions\s+Read\s+and\s+write",
    ),
)

require_readme_patterns(
    "README.md は --publish / --no-publish と dispatch_build_release の関係を説明してください",
    (
        r"setup\.sh\s+all\s+--publish",
        r"setup\.sh\s+all\s+--no-publish",
        r"github\.dispatch_build_release\s*=\s*false",
        r"github\.dispatch_build_release\s*=\s*true",
    ),
)
...
run_publish_case "no-publish" "success" "0" "0" "true" "stub-token" "0" "1"
run_publish_case "dry-run" "success" "1" "auto" "true" "stub-token" "0" "1"
run_publish_case "auto-disabled" "success" "0" "auto" "false" "stub-token" "0" "1"
run_publish_case "auto-enabled-success" "success" "0" "auto" "true" "stub-token" "0" "1"
run_publish_case "publish-force-success" "success" "0" "1" "false" "stub-token" "0" "1"
run_publish_case "token-missing" "success" "0" "1" "false" "" "1" "1"
run_publish_case "workflow-get-fail" "workflow-get-fail" "0" "1" "false" "stub-token" "1" "1"
run_publish_case "dispatch-fail" "dispatch-fail" "0" "1" "false" "stub-token" "1" "1"
