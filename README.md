...
０２.[PXVIRT（Proxmox）](https://docs.pxvirt.lierfang.com/en/README.html)導入（Linux側操作）<br>
```
...
~/Linux/platforms/scripts/minecraft_conversion1.sh
...
~/Linux/platforms/scripts/minecraft_stop.sh
~/Linux/platforms/scripts/minecraft_conversion2.sh

【resourceワールドのバックアップ作成】
「/mnt/share/kubernetes/minecraft」上の「server1」「server2」をバックアップから各々復元
~/Linux/platforms/scripts/minecraft_start.sh
...
PATH=/bin/bash:/usr/local/sbin:/usr/bin:/bin:/usr/local/bin
# JOB_NET設定
00 3 * * 2 ~/Linux/platforms/scripts/kubernetes_cron1.sh && ~/Linux/platforms/scripts/update.sh
...
～サーバーのバージョンアップ対応～
    ...
    【ymlファイル編集】
    ~/Linux/platforms/scripts/minecraft_stop.sh
    「/mnt/share/kubernetes/minecraft」上の「server1」「server2」のバックアップを取得
