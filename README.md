...
０６.クラスタ作成（Windows_Proxmox 側操作）<br>
...
０７.クラスタ参加（Windows_Proxmox 側操作）<br>
...
hostpci0: 0000:★⑤内容
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
「データセンター」→「pve-XX」→「VM」→「ハードウェア」→「追加」→「USBポートを使用」→「バックアップ先のHDDを指定」→「追加」
VMを起動
以下を設定
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
「Enter」
...
０５.[Tailscale](https://tailscale.com/)導入（Windows_TereTerm（VM）側操作）<br>
...
０６.GitHub 設定（Windows_TereTerm（Node、VM）側操作）<br>
...
０７.docker 導入（Windows_TereTerm（VM（k8s 環境全て））側操作）<br>
...
０８.[Kubernetes](https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/install-kubeadm/#installing-kubeadm-kubelet-and-kubectl)導入（Windows_TereTerm（VM（k8s 環境全て））側操作）<br>
...
１７.NFS クライアント設定（Windows_TereTerm（VM（ubuntu-102 以外））側操作）<br>

```
sudo apt install -y nfs-common
```

１８.NFS サーバ設定（Windows_TereTerm（VM（ubuntu-102））側操作）<br>

```
sudo apt install -y nfs-kernel-server
sudo mkdir -p /mnt/share/kubernetes/{alertmanager,grafana,mariadb-phpmyadmin,minecraft,navidrome,prometheus,wordpress}
...
navidrome表示内容にてログイン
  ※navidromeはユーザー名：root
rootユーザーにて、以下を設定
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
「右上アイコン」→「Personal」→「Language」→「日本語」
「右上アイコン」→「ユーザー」→「作成」→「ユーザー名：Gamer-Iris、名前：Gamer-Iris、パスワード：任意のパスワード、管理者：チェック」→「保存」
...
２８.minecraft 設定（Windows_TereTerm（VM（ubuntu-102）及び minecraft）側操作）<br>
